...
export async function GET(req) {
  try {
    ...
    const MemberName = await pool
      .request()
      .query(
        "SELECT m.memberId, m.CNICNo, m.MemberName, m.MemberFatherName, PicPath, m.FamilyID, f.FamilyName FROM tb_member_mst AS m LEFT JOIN tb_member_family_mst AS f ON m.FamilyID = f.FamilyID;"
      );
